...
public class EmailPassLogin : MonoBehaviour
{
    ...
    private void Start()
    {
        if (PlayerPrefs.GetInt("LogedIn", 0) == 1)
        {
            showLogMsg("Already Logged in");
            Invoke(nameof(AlreadyLogedIN), 2f);
    ...
    public void SignUp()
    {
        ...
        auth.CreateUserWithEmailAndPasswordAsync(email, password).ContinueWithOnMainThread(task => {
            ...
            if (task.IsFaulted)
            {
                ...
                ErrorTxt.text = "Creating User With Email And Password encountered an error..";
                Invoke(nameof(disableMessag), 3f);
    ...
    public void Login()
    {
        ...
        auth.SignInAndRetrieveDataWithCredentialAsync(credential).ContinueWithOnMainThread(task => {
            ...
            if (task.IsFaulted)
            {
                ...
                ErrorBar.SetActive(true);
                ErrorTxt.text = "Loging User With Email And Password encountered an error..";
                Invoke(nameof(disableMessag), 3f);
